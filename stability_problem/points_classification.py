...
def classify_system(tr, det):
    #return None
    disc = tr**2 - 4*det
    if det < 0: return "Saddle", "#e74c3c"
    if disc < 0:
        if abs(tr) < 0.001: return "Center", "#9b59b6"
        return ("Stable Spiral" if tr < 0 else "Unstable Spiral"), "#3498db"
    return ("Stable Node" if tr < 0 else "Unstable Node"), "#2ecc71"
...
class VectorizedLab:
    ...
    def draw_phase_portrait(self):
        ...
        # Update Title
        cls_result = classify_system(self.tr, self.det)
        if cls_result:
            label, color = cls_result
        ...
        def odefunc(t, y_flat):
            ...
            dx, dy = get_derivatives(y_grid[0], y_grid[1], self.tr, self.det)
            return np.concatenate([dx, dy])

        res_check = get_derivatives(self.seeds[0], self.seeds[1], self.tr, self.det)
    ...
    def draw_map(self):
        ...
        try:
            # 1. Vectorize to get both Label and Color
            # We use 'object' output to handle the tuple return
            v_classify = np.vectorize(classify_system, otypes=[object])
            results = v_classify(t_grid, d_grid)
            
            # If student returns None, results will be None
            if results[0,0] is None:
                raise ValueError("Not implemented")

            # 2. Extract labels and colors into separate arrays
            labels = np.vectorize(lambda x: x[0])(results)
            colors = np.vectorize(lambda x: x[1])(results)

            # 3. Create the mapping for imshow
            unique_labels = np.unique(labels)
            
            # Build a list of colors that corresponds to the order of unique_labels
            # We grab the first color encountered for each unique label
            color_palette = []
            label_to_id = {}
            
            for i, label in enumerate(unique_labels):
                label_to_id[label] = i
                # Find first instance of this label to get its associated color
                sample_color = colors[labels == label][0]
                color_palette.append(sample_color)

            # 4. Create a custom Matplotlib colormap from student colors
            from matplotlib.colors import ListedColormap
            student_cmap = ListedColormap(color_palette)

            # 5. Map labels to IDs and display
            ...
            # If classify_system returns None or is broken, show no background
            pass
        finally:
