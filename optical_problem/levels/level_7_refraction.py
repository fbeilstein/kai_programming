...
class TestLevel7Refraction(unittest.TestCase):
    ...
    def test_l7_refraction_segment_limit(self):
        """Verify refraction only occurs if the ray hits the finite segment."""
        # Ray aimed at [10, 0], which is the midpoint of segment [10, -20] to [10, 20]
        origin = np.array([0.0, 0.0])
        rd = np.array([1.0, 0.0])
        p1 = np.array([10.0, -20.0])
        p2 = np.array([10.0, 20.0])
        
        hit = tasks.intersect_segment(origin, rd, p1, p2)
        self.assertIsNotNone(hit, "Should hit the segment.")
        
        # Ray aimed at [10, 30], which hits the infinite line but misses the segment
        rd_miss = np.array([1.0, 3.0]) / np.sqrt(10)
        hit_miss = tasks.intersect_segment(origin, rd_miss, p1, p2)
        self.assertIsNone(hit_miss, "Should not refract if missing the finite segment.")
